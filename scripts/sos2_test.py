...
y_low = points["wind"]["y_fit"].min()
y_high = points["wind"]["y_fit"].max()
x_low = n.carriers.loc[learn_i, "global_capacity"].loc["wind"]
x_high = n.carriers.loc[learn_i, "max_capacity"].loc["wind"]
######################################################################
m = Model("test_SOS2")
...
x_link = m.addConstr(quicksum([weights[i]*points["wind"]["x_fit"][i] for i in range(len(points))]) == x)
y_link =  m.addConstr(quicksum([weights[i]*points["wind"]["y_fit"][i] for i in range(len(points))]) == y)
################################################################
# Set the objective
