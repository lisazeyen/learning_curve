...
def _export_to_exporter(network, exporter, basename, export_standard_types=False):
    ...
    if isinstance(network.snapshot_weightings.index, pd.MultiIndex):
        network.snapshot_weightings.index.rename(
            ["investment_periods", "snapshots"], inplace=True
        )
    else:
        network.snapshot_weightings.index.rename("snapshots", inplace=True)
...
def _import_from_importer(network, importer, basename, skip_time=False):
    ...
    if df is not None:
        ...
        if investment_periods is not None:
            network.investment_period_weightings = investment_periods.reindex(
                network.investment_period_weightings.index
            )
        if all(network.snapshot_weightings.columns.isin(df.columns)):
            network.snapshot_weightings = df[
                network.snapshot_weightings.columns
            ].reindex(network.snapshots)
        elif "weightings" in df.columns:
            logger.warning(
                "snapshot weightings are in the current "
                "version split between objective weightings "
                "and component weightings"
            )
            network.snapshot_weightings = network.snapshot_weightings.apply(
                lambda x: df.weightings.reindex(network.snapshots).fillna(1.0)
            )
    imported_components = []
...
def import_components_from_dataframe(network, dataframe, cls_name):
    ...
    if cls_name == "Generator" and "dispatch" in dataframe.columns:
        logger.warning(
            "'dispatch' for generators is deprecated, use time-varing 'p_max_pu' for 'variable' and static 'p_max_pu' for 'flexible'."
        )
    if (
        cls_name in ["Generator", "StorageUnit"]
        and "p_max_pu_fixed" in dataframe.columns
    ):
        logger.warning(
            "'p_max_pu_fixed' for generators is deprecated, use static 'p_max_pu' instead."
        )
    if (
        cls_name in ["Generator", "StorageUnit"]
        and "p_min_pu_fixed" in dataframe.columns
    ):
        logger.warning(
            "'p_min_pu_fixed' for generators is deprecated, use static 'p_min_pu' instead."
        )
...
def import_series_from_dataframe(network, dataframe, cls_name, attr):
    ...
    # ]
    dataframe = dataframe.reindex_like(pnl[attr]).fillna(default)
    pnl[attr] = dataframe
