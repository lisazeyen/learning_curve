...
def calculate_costs(n, label, costs):
    ...
    for c in n.iterate_components(
        ...
        active = pd.concat(
            [
                get_active_assets(n, c.name, inv_p, n.snapshots).rename(inv_p)
...
def calculate_capital_costs_learning(n, label, df):
    investments = n.snapshots.levels[0]
    cols = pd.MultiIndex.from_product(
        [df.columns.levels[0], df.columns.levels[1], df.columns.levels[2], investments],
        names=df.columns.names[:3] + ["year"],
    )
    df = df.reindex(cols, axis=1)
...
def make_summaries(networks_dict):
    ...
    for label, filename in iteritems(networks_dict):
        ...
        try:
            n = pypsa.Network(
                filename, override_component_attrs=override_component_attrs
            )
        except OSError:
...
if __name__ == "__main__":
    ...
    if "snakemake" not in globals():
        ...
        import yaml
        snakemake = Dict()
        with open('/home/lisa/mnt/lisa/learning_curve/results/sos2_timedelay_moreinvp_notimedelay/configs/config.yaml', encoding='utf8') as f:
    ...
    networks_dict = {
        ...
        + "/postnetworks/elec_s_EU_{sector_opts}.nc".format(  # "results/" + snakemake.config['run'] +"/postnetworks/elec_s_{clusters}_lv{lv}_{sector_opts}.nc"\
            # clusters=clusters,
            # lv=lv,
