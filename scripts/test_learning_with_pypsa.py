...
def learning_consistency_check(n):
    ...
    (0) are there any learning technologies?
...
def cumulative_cost_curve(cumulative_capacity, learning_rate, c0, initial_capacity=1):
    ...
    return (c0/(1-alpha)) * cumulative_capacity * ((cumulative_capacity/initial_capacity)**-alpha)
...
def get_cumulative_cap_from_cum_cost(cumulative_cost, learning_rate, c0, e0):
    ...
    return (cumulative_cost * (1-alpha) * (e0**-alpha) /c0 )**(1/(1-alpha))
...
def piecewise_linear(y, segments, learning_rate, c0, e0, carrier):
    ...
    data = [y.min()] + [y.min() + int(initial_len)*2**s for s in range(segments)]
    y_fit = pd.Series(data, index=np.arange(segments+1))
...
def define_learning_binary_constraint(n, snapshots):
    ...
    # experience must grow constraints (p.67 Barretto, eq 19)
...
def define_x_position(n, x_low, x_high, investments, multi_i, learn_i, points,
                      segments):
    """
    defines variable "xs" (in Barretto called "lambda") and constraints for
    x-position on stepwise-linearised learning curve
    (=cumulative capacity until given investment period for  each learning
     technology and line segment)

    this approach is based on Mattson (1997)
    """
    c = "Carrier"
    # ---- DEFINE VARIABLE
    # define xs capacity variables for each line segment, technology, investment_period
    xs = define_variables(n, 0, x_high, c, "xs",
    ...
    # define lower and upper boundares for cumulative capacity at each line segment
    # in heuberger et. al. p.6 eq (40,41)
    # in Barretto p.66 eq (17) lambda=xs, delta=learning
    x_lb = (expand_series(points.stack(-2)["x_fit"], investments).T
            .swaplevel(axis=1).drop(segments, axis=1, level=1))
    lhs =  linexpr((1, xs),
                   (-x_lb, learning))

    define_constraints(n, lhs, '>=', 0, 'Carrier', 'xs_lb')

    x_ub = (expand_series(points.stack(-2)["x_fit"], investments).T
            .swaplevel(axis=1).shift(-1, axis=1).dropna(axis=1))
    lhs =  linexpr((1, xs),
                   (-x_ub, learning))
...
def define_capacity_per_period(n, investments, multi_i, learn_i, points, segments):
    """
    defines variable and constraint for new installed capacity per investment
    period
    """
    c = "Carrier"
    x_lb = (expand_series(points.stack(-2)["x_fit"], investments).T
    ...
    rhs.iloc[0] = n.carriers.global_capacity
...
def define_cumulative_cost(n, points, investments, segments, learn_i):
    ...
    y_lb = (expand_series(points.stack(-2)["y_fit"], investments).T
            .swaplevel(axis=1).drop(segments, axis=1, level=1))
    y_ub = (expand_series(points.stack(-2)["y_fit"], investments).T
            .swaplevel(axis=1).shift(-1, axis=1).dropna(axis=1))
    ...
    slope_t = expand_series(slope.stack(), investments).swaplevel().T
    y_intercept = get_interception(points, slope)
    y_intercept_t = expand_series(y_intercept.stack(), investments).swaplevel().T
    # Variables ---
    # capacity at each line segment
    xs = get_var(n, c, "xs")
    # learning binaries
...
def define_position_on_learning_curve(n, snapshots, segments=5):
    ...
    # get learning binaries ------------------------------------------------
    learning = get_var(n, "Carrier", "learning")
    ...
    # ######## CAPACITY #######################################################
    # -------- define xs/lambda capacity per line segment  --------------------
...
n.generators.p_nom_opt.plot(kind="bar")
#%% debug
import gurobipy
